...
describe('DELETE /api/v1/urls', () => {
  ...
  describe('Permanent Delete', () => {
    it('should permanently delete a trashed url', async () => {
      const { token } = await createAuthenticatedUser();
      ...
      const response = await request(app)
        .delete(`/api/v1/urls/id/${url._id}/permanent`)
        .set('Authorization', `Bearer ${token}`);
    ...
    it('should return 404 if url is not in trash', async () => {
      const { token } = await createAuthenticatedUser();
    ...
    it("should not permanently delete another user's url", async () => {
      const owner = await createAuthenticatedUser();
      const attacker = await createAuthenticatedUser();
      ...
      const stillExists = await UrlModel.findOne({
        _id: url._id,
      });
    ...
    it('should require authentication', async () => {
      ...
      const response = await request(app).delete(`/api/v1/urls/id/${url._id}/permanent`);
