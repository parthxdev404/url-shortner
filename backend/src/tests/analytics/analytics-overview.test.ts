...
describe('GET /api/v1/analytics/:id/analytics', () => {
  it('should return analytics for a url', async () => {
    ...
    expect(Array.isArray(response.body.data.recentClicks)).toBe(true);

    expect(response.body.data.recentClicks).toHaveLength(2);
  });

  it('should return empty analytics when none exist', async () => {
    const { token } = await createAuthenticatedUser();
    ...
    expect(response.body.data.recentClicks).toEqual([]);
  });
  ...
  it('should return analytics ordered by latest click first', async () => {
    ...
    expect(new Date(response.body.data.recentClicks[0].clickedAt).getTime()).toBeGreaterThan(
      new Date(response.body.data.recentClicks[1].clickedAt).getTime(),
    );
  });
  ...
  it('should return all analytics belonging only to requested url', async () => {
    ...
    expect(response.body.data.recentClicks).toHaveLength(1);

    expect(response.body.data.recentClicks[0].urlId).toBe(url1._id.toString());
  });
});
